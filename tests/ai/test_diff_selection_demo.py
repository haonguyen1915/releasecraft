...
def test_breaking_changes_always_need_diffs():
    ...
    commits = [
        ...
        {
            "hash": "abc2",
            "message": "fix(auth)!: update token validation",
            "author": "Dev",
            "date": "2024-01-01",
        },
        {
            "hash": "abc3",
            "message": "refactor: BREAKING CHANGE: remove deprecated methods",
            "author": "Dev",
            "date": "2024-01-01",
        },
    ]
...
def test_security_commits_always_need_diffs():
    ...
    commits = [
        ...
        {
            "hash": "abc2",
            "message": "patch CVE-2024-1234",
            "author": "Dev",
            "date": "2024-01-01",
        },
        {
            "hash": "abc3",
            "message": "fix: prevent XSS attack vector",
            "author": "Dev",
            "date": "2024-01-01",
        },
        {
            "hash": "abc4",
            "message": "feat: add SQL injection protection",
            "author": "Dev",
            "date": "2024-01-01",
        },
    ]
...
def test_mixed_scenario():
    ...
    commits = [
        {
            "hash": "abc1",
            "message": "feat: add comprehensive OAuth2 authentication",
            "author": "Dev",
            "date": "2024-01-01",
        },  # Clear, no diff
        {
        ...
        {
            "hash": "abc5",
            "message": "fix: resolve security issue in token handling",
            "author": "Dev",
            "date": "2024-01-01",
        },  # Security, needs diff
        {
    ...
    for commit in commits:
        ...
        if "!" in commit["message"].split(":")[0]:
            ...
            any(
                ...
                for word in ["update", "change", "changes"]
            )
            and len(commit["message"].split(":")[-1].strip().split()) <= 2
        ):
            reason = "(vague)"
